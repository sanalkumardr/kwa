-- =====================================================================
-- KWA Pipeline Works — RLS completion + seed data + bill demo
-- Run AFTER kwa_schema.sql.
--   Part A: enable RLS + project-scope policy on every project-scoped table
--   Part B: seed reference data (org, users, SOR edition, deduction scheme)
--   Part C: seed one project end-to-end and compute a bill
-- =====================================================================
...
-- project and mb_entry had RLS enabled in the schema; FORCE them so the owner
-- is also subject to the policies (owners bypass RLS otherwise).
...
DECLARE t text;
BEGIN
  -- bill_line / bill_deduction / payment are handled below — they have no
  -- project_id at this point, so including them here would fail policy creation.
...
-- bill_line / bill_deduction reference a bill, not a project directly.
-- Replace their project_id-based policy with a bill-derived one.
...
-- (If you prefer no denormalization, scope via bill instead:)
